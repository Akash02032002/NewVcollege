  ...
  fetch("index.php?action=view_college&id=" + id)
    ...
    .then((data) => {
      if (data.success) {
        ...
        if (c.college_image) {
          imgHtml =
            '<img src="../uploads/colleges/' +
            escHtml(c.college_image) +
            '" class="modal-clg-img" alt="">';
        }
...
function detailRow(label, value) {
  return (
    '<div class="clg-detail-row"><div class="clg-detail-label">' +
    label +
    '</div><div class="clg-detail-value">' +
    value +
    "</div></div>"
  );
}
...
function courseBadges(courses) {
  ...
  return courses
    .split(",")
    .map((c) => '<span class="course-badge">' + escHtml(c.trim()) + "</span>")
    .join(" ");
}
